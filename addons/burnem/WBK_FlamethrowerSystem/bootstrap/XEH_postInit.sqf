	...
	) exitWith 
	{
		...
		[_1_fum,_object,_blow_obj] spawn 
		{
			...
			while {alive _1_fum} do 
			{
				...
				_startPos = getposasl _blow_obj;
				_dir = _object weaponDirection currentWeapon _object;
				_endPos = _startPos vectorAdd (_dir vectorMultiply _al_pressure_1);
				...
				if (count _hits > 0) then {
					...
					_distance = getposasl _1_fum distance _hitPos;
					_flow_1 = _dir vectorMultiply _distance;
				};
				_params = [["\A3\data_f\cl_exp", 1, 0, 1],
				"",
				"Billboard",
				1,
				0.8,
				...
				_1_fum setParticleParams _params;
				uiSleep 0.1;
			};
		};
		sleep 0.4;
